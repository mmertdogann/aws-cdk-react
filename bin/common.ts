...
import { getConfig, getAppEnv } from '../lib/config';
import { AssetsBucketStack } from '../lib/s3';
...
new AssetsBucketStack(app, `AssetsBucketStack-${appEnv}`, {
  env: frankfurtEnv,
});

//Create certificate for both Frankfurt and N.Virgina regions
new CertificateManagerStack(app, 'CertificateManagerStack-frankfurt', {
  env: frankfurtEnv,
});
new CertificateManagerStack(app, 'CertificateManagerStack-virginia', {
  env: virginiaEnv,
});

new AssetsCloudfrontDistributionStack(
  app,
  `AssetsCloudfrontDistributionStack-${appEnv}`,
  { env: virginiaEnv }
);
